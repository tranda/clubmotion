...
export default function LedgerCategories({ categories }) {
    ...
    const submit = (e) => {
        e.preventDefault();
        if (editing) {
            form.put(`/ledger/categories/${editing.id}`, {
                preserveScroll: true,
                onSuccess: cancel,
            });
        } else {
            form.post('/ledger/categories', {
                preserveScroll: true,
        ...
        <Layout>
            <div className="py-4 max-w-3xl mx-auto">
                <div className="mb-4 flex justify-between items-center">
                    <h1 className="text-2xl font-bold text-gray-800">Ledger categories</h1>
                ...
                <div className="bg-white rounded-lg shadow overflow-x-auto">
                    <table className="min-w-full text-sm">
                        ...
                        <tbody>
                            {categories.length === 0 && (
                                <tr><td colSpan="6" className="px-3 py-6 text-center text-gray-400">No categories yet.</td></tr>
                            )}
                            {categories.map((c) => (
                                <tr key={c.id} className="border-t border-gray-100 hover:bg-gray-50">
                                    <td className="px-3 py-2">{c.name}</td>
                                    <td className="px-3 py-2 capitalize">{c.kind}</td>
                                    <td className="px-3 py-2 text-right">{c.entries_count ?? 0}</td>
